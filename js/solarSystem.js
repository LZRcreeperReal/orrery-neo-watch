...
import { PLANETS, heliocentricPosition, orbitPath, zodiacForLongitude, PLANET_FACTS } from './orbitalElements.js';

const AU = 22;                 // scene units per astronomical unit (orbits are to scale)
const SPEED_DAYS_PER_SEC = [0.2, 1, 5, 20, 80];
const SPEED_LABELS = ['0.2 d/s', '1 d/s', '5 d/s', '20 d/s', '80 d/s'];

const SYNODIC_MONTH_DAYS = 29.530588853;
const KNOWN_NEW_MOON_MS = Date.UTC(2000, 0, 6, 18, 14); // a documented reference new moon
...
let playing = true;
let speedIndex = 1; // index into SPEED_DAYS_PER_SEC — 1 day of sim time per real second
...
/** Geocentric ecliptic longitude -> zodiac constellation, using cached per-frame positions. */
function currentZodiac(def) {
  const earthBody = bodies.find(b => b.def.key === 'earth');
  const earthPos = earthBody?.mesh.userData.pos;
  if (!earthPos) return null;

  let dx, dy;
  if (def.key === 'earth') {
    // "Earth" card shows where the Sun currently sits in the sky as seen from Earth
    dx = -earthPos.x;
    dy = -earthPos.y;
  } else {
    const body = bodies.find(b => b.def.key === def.key);
    const pos = body?.mesh.userData.pos;
    if (!pos) return null;
    dx = pos.x - earthPos.x;
    dy = pos.y - earthPos.y;
  }
  const lonDeg = Math.atan2(dy, dx) * 180 / Math.PI;
  return zodiacForLongitude(lonDeg);
}

function factsRows(def) {
  const facts = PLANET_FACTS[def.key];
  if (!facts) return '';
  return `
    <div class="planet-card__row"><span>Known moons</span><span>${facts.moons}</span></div>
    <div class="planet-card__row"><span>Surface gravity</span><span>${facts.gravity}</span></div>
    <div class="planet-card__row"><span>Day length</span><span>${facts.dayLength}</span></div>
    <div class="planet-card__row"><span>Avg. temperature</span><span>${facts.avgTemp}</span></div>
  `;
}
...
function selectPlanet(key) {
  ...
  const zodiac = currentZodiac(body.def);

  panel.innerHTML = `
    <h2 class="planet-card__name">${body.def.name}</h2>
    <span class="planet-card__type">Planet</span>
    ...
    ${zodiac ? `<div class="planet-card__row"><span>${key === 'earth' ? 'Sun currently in' : 'Currently in'}</span><span>${zodiac}</span></div>` : ''}
    ${factsRows(body.def)}
    <p class="planet-card__footnote">Zodiac position is geocentric — as seen from Earth, right now.</p>
  `;
}
...
function selectSun() {
  ...
  panel.innerHTML = `
    ...
    <div class="planet-card__row"><span>Surface temp.</span><span>~5,500°C</span></div>
...
function moonPhase(date) {
  ...
  const name = MOON_PHASE_NAMES[Math.round(frac * 8) % 8];
...
function updateHud() {
  ...
  const moonEl = document.getElementById('moonReadout');
  if (moonEl) {
    const mp = moonPhase(simTime);
    moonEl.textContent = `${mp.name} · ${mp.illumination}%`;
  }
}
...
function wireControls() {
  ...
  // JS is the source of truth for the initial speed — sync the controls to it
  // rather than trusting hardcoded markup, which is what caused "1x" to
  // silently mean something else in the past.
  if (speedSlider) speedSlider.value = String(speedIndex);
  ...
  // spacebar toggles play/pause while the solar system tab is active,
  // unless the user is typing in a text field somewhere
  window.addEventListener('keydown', (e) => {
    if (e.code !== 'Space') return;
    const tag = document.activeElement?.tagName;
    if (tag === 'INPUT' || tag === 'TEXTAREA') return;
    if (!document.getElementById('view-solar')?.classList.contains('is-active')) return;
    e.preventDefault();
    playPauseBtn?.click();
  });
}
